...
class WrapperRunner extends BaseRunner
{
  ...
  protected function load()
  {
    if($this->options->functional)
    {
      throw new \RuntimeException(
        "The `functional` option is not supported yet in the WrapperRunner. Only full classes can be run due to the current PHPUnit commands causing classloading issues."
      );
    }
    parent::load();
  }
  ...
  private function startWorkers()
  {
    ...
    for($i = 1; $i <= $this->options->processes; $i++)
    {
      ...
      {
        $fileHandle = fopen("paratest-{$token}.log", "wr") or die("Unable to open file: paratest-{$token}.log");
      }

      $worker->start($wrapper, $token, $uniqueToken, $fileHandle);
  ...
  private function setExitCode()
  {
    ...
    else if($this->interpreter->getTotalFailures())
    {
